...
def log_operation(log_path, timestamp, command, moved_items):
...
def move_to_trash(
    ...
    if not os.path.exists(file_path):
        if not force:
            print(
                f"{EXE_NAME}: cannot remove '{file_path}': No such file or directory."
            )
        return
    ...
    # Confirm deletion if interactive mode is enabled and not in force mode
    if not force and interactive:
        if not confirm_delete(file_path): # Not confirmed
...
def main():
    ...
    if moved_items:
        log_operation(log_file_path, timestamp, command, moved_items)
